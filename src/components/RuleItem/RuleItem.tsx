...
const RuleItem = ({ rule, index }: Props) => {
  ...
  const { editGameRule, deleteGameRule } = useMutateGameRule({
    ...
    onSuccessDeleteGameRule(ruleId) {
  ...
  return (
    <div className="flex items-center justify-between w-full max-w-screen-sm  my-4 flex-wrap border-[1px] border-solid border-slate-400 px-2 rounded-sm text-sm">
      <b>Rule {index + 1}:</b>
      <Input
        ...
        onChange={(e) => {
          setUpdatedRule(
            produce((draft) => {
              draft.divisibleNumber = Number(e.target.value);
            })
          );
        }}
      ...
      <Button
        ...
        disabled={!isRuleChanged || editGameRule.isPending}
      >
